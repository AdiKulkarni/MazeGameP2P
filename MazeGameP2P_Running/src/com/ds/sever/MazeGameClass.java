...
public final class MazeGameClass implements Serializable {
	...
	public synchronized int getNewMoveLocation(int oldLocation, String moveDir) {
		final Object lock = new Object();
		int newLoc;
		switch (moveDir.toUpperCase()) {
		case "A":
			synchronized (lock) {
				newLoc = oldLocation - 1;
				return newLoc;
			}

		case "W":
			synchronized (lock) {
				newLoc = oldLocation - SIZE_OF_MAZE;
				return newLoc;
			}
		case "S":
			synchronized (lock) {
	...
	public MazeGameClass move(int pId, String direction) {
		...
		case "A":
			if (oldLocation % SIZE_OF_MAZE != 0) {
				newLocation = getNewMoveLocation(oldLocation, "A");// oldLocation
																	// - 1;
		...
		case "W":
			if (oldLocation > SIZE_OF_MAZE - 1) {
				newLocation = getNewMoveLocation(oldLocation, "W");// oldLocation
																	// -
																	// SIZE_OF_MAZE;
				if (!mCells[newLocation].isOccupied()) {
					NO_OF_TREASURES_LEFT = NO_OF_TREASURES_LEFT
		...
		case "S":
			if (oldLocation < (SIZE_OF_MAZE * SIZE_OF_MAZE - SIZE_OF_MAZE)) {
				newLocation = getNewMoveLocation(oldLocation, "S");// oldLocation
																	// +
																	// SIZE_OF_MAZE;
				if (!mCells[newLocation].isOccupied()) {
					NO_OF_TREASURES_LEFT = NO_OF_TREASURES_LEFT
		...
		case "D":
			if (oldLocation % SIZE_OF_MAZE != SIZE_OF_MAZE - 1) {
				newLocation = getNewMoveLocation(oldLocation, "D");// oldLocation
																	// + 1;
				if (!mCells[newLocation].isOccupied()) {
					NO_OF_TREASURES_LEFT = NO_OF_TREASURES_LEFT
							- mCells[newLocation].getNoOfTreasuresInaCell();
